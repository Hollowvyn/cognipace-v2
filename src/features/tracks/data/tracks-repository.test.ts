...
describe('TracksRepository', () => {
  ...
  it('orders groups and memberships by their persisted positions', async () => {
    const handle = await createTestDb({
      now: new Date('2026-01-01T00:00:00.000Z'),
    })
    const timestamp = new Date('2026-01-01T08:00:00.000Z').getTime()
  ...
  it('writes active track and clear-session ids', async () => {
    const handle = await createTestDb({
      now: new Date('2026-01-01T00:00:00.000Z'),
    })
    const repository = createTracksRepository(handle.db)
    ...
    const activated = await repository.setActiveTrack(
      'grind-75',
    ...
    const cleared = await repository.clearActiveTrack(
  ...
  it('records completion only for first incomplete active-track membership', async () => {
    const handle = await createTestDb({
      now: new Date('2026-01-01T00:00:00.000Z'),
    })
    const timestamp = new Date('2026-01-01T08:00:00.000Z').getTime()
