...
export async function validateStoreBuild({ rootDir = process.cwd() } = {}) {
  ...
  if (manifest && packageJson) {
    ...
    if (manifest.name !== EXPECTED_NAME) errors.push(`name must be exactly "${EXPECTED_NAME}"`);
  ...
  if (manifest) {
    ...
    for (const [label, iconMap] of maps) {
      for (const size of REQUIRED_ICON_SIZES) {
        if (!iconMap || typeof iconMap !== 'object' || !iconMap[size]) {
      ...
      if (iconMap && typeof iconMap === 'object') {
        for (const value of Object.values(iconMap)) {
          ...
          if (normalized) iconFiles.add(normalized);
        }
      }
    }
  }
  ...
  if (errors.length > 0) {
    throw new Error(`Store build validation failed:\n${errors.map((error) => `- ${error}`).join('\n')}`);
  }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    ...
    console.log(`Store build validation passed for CogniPace ${result.version} with ${result.iconFiles.length} icon files.`);
  } catch (error) {
